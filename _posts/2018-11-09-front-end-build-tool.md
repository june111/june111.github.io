...
* NPM
	...
	* 优点
		* 社区成熟
		* 通过冗余安装来解决套件相依性问题。如果要安装套件 A 和套件 B，而两者都相依于套件 C，则 npm 会在套件 A 和套件 B 的 node_modules 资料夹内同时安装套件 C。换句话说，套件 C 被安装了两次。这种设计的好处是不会有相依性问题。

	* 缺点
		* 同样的套件会被安装多次，当有需要编译的套件被冗余安装多次时，更新会变得很缓慢，幸好绝大多数的套件都不需要编译。
...
* Webpack
	...
	* 缺点
		* 上手有难度
		* 只能用于采用模块化开发的项目：对于 Server 端渲染的多页应用有点力不从心，Webpack 的最初设计就是针对 SPA，所以在处理 Server 端渲染的多页应用时，不管你如何 chunk，总不能真正达到按需加载的地步，往往要去考虑如何提取公共文件才能达到最优状态
